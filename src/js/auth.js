...
class AuthService {
    ...
    renderOTPCard(otpCard) {
        try {
            ...
            // 创建OTP卡容器
            const otpCardContainer = document.createElement('div');
            otpCardContainer.className = 'otp-card-container';
            ...
            for (let i = 0; i < 10; i++) {
                ...
                for (let j = 0; j < 10; j++) {
                    ...
                    // 如果是对象格式，使用key值获取
                    if (isObjectFormat) {
                        otpValue = otpCard[`${i}_${j}`];
                    }
                    // 如果是数组格式
                    else if (Array.isArray(otpCard) && Array.isArray(otpCard[i])) {
                        otpValue = otpCard[i][j];
                    }
                    
                    // 格式化OTP值，每两个字符添加空格，提高可读性
                    if (otpValue && otpValue.length > 2) {
                        let formattedValue = '';
                        for (let k = 0; k < otpValue.length; k++) {
                            formattedValue += otpValue[k];
                            if (k % 2 === 1 && k !== otpValue.length - 1) {
                                formattedValue += ' ';
                            }
                        }
                        td.textContent = formattedValue;
                    } else {
                        td.textContent = otpValue;
                    }
                    
                    // 添加坐标属性，方便后续引用
                    td.setAttribute('data-coord', `${String.fromCharCode(65 + i)}${j}`);
            ...
            otpCardContainer.appendChild(table);
            
            // 添加打印按钮
            const printButtonContainer = document.createElement('div');
            printButtonContainer.style.textAlign = 'center';
            printButtonContainer.style.marginTop = '15px';
            
            const printButton = document.createElement('button');
            printButton.className = 'hacker-btn';
            printButton.textContent = '打印OTP卡';
            printButton.onclick = () => {
                    ...
                    <html>
                        <head>
                            ...
                            <style>
                                ...
                                .otp-table { border-collapse: collapse; width: 100%; max-width: 800px; margin: 0 auto; }
                                .otp-table th { background-color: #333; color: #0f0; padding: 8px; text-align: center; }
                                .otp-table td { border: 1px solid #333; padding: 8px; text-align: center; font-family: monospace; }
                                h2 { text-align: center; color: #0f0; margin-bottom: 30px; }
                                .otp-description { margin: 20px auto; max-width: 600px; text-align: center; }
                                .warning { color: #f00; font-weight: bold; margin: 20px 0; text-align: center; }
                                @media print {
                                    body { background: white; }
                                    .otp-table th { background-color: #ccc !important; color: black !important; }
                        ...
                        <body>
                            <h2>SECRET_CHAT OTP验证卡</h2>
                            <div class="otp-description">
                                <strong>使用说明：</strong><br>
                                当登录时系统会随机要求提供卡片上指定位置的验证码<br>
                                例如：请提供B3位置的验证码，您需要在表格中找到B行和3列的交叉处的数字
                            </div>
                            ${table.outerHTML}
                            <div class="warning">请安全保存此OTP卡，不要与他人分享！</div>
                        </body>
                    </html>
                `);
            ...
            otpCardContainer.appendChild(printButtonContainer);
            
            // 添加到DOM
            otpCardElement.appendChild(otpCardContainer);
        } catch (error) {
